...
// ── Drop rates ────────────────────────────────────────────────────────────────
const _dropRates = {
  Rarity.common: 50, Rarity.uncommon: 28,
  Rarity.rare: 14, Rarity.epic: 6, Rarity.legendary: 2,
};
const _dropLabels = {
  Rarity.common: '50%', Rarity.uncommon: '28%',
  Rarity.rare: '14%', Rarity.epic: '6%', Rarity.legendary: '2%',
...
SavedCard _weightedPick(List<SavedCard> pool, math.Random rng) {
  ...
  for (final c in pool) { byR.putIfAbsent(c.rarity, () => []).add(c); }
  ...
  for (final r in [Rarity.common, Rarity.uncommon, Rarity.rare,
    ...
    roll -= _dropRates[r]!;
    if (roll < 0) { final p = byR[r]!; return p[rng.nextInt(p.length)]; }
  }
...
// Clé cartes obtenues liée à l'utilisateur
String _obtKey(String colId) {
  final uid = Supabase.instance.client.auth.currentUser?.id ?? 'anon';
  return 'obtained_${uid}_$colId';
}
// Clé catalogue local (fallback si Supabase échoue)
...
class CollectionDetailScreen extends StatefulWidget {
  ...
  const CollectionDetailScreen({
    super.key, required this.collection, required this.myUserId});
...
class _CollectionDetailScreenState extends State<CollectionDetailScreen>
  ...
  Future<void> _loadCards() async {
    ...
    // Cartes obtenues
    final obtIds = prefs.getStringList(_obtKey(widget.collection.id)) ?? [];

    // Catalogue : Supabase + fallback local
    Set<String> catIds = {};
    try {
      final remote = await CollectionService.instance
          .getCollectionCardIds(widget.collection.id);
      catIds.addAll(remote);
    } catch (_) {}
    // Ajoute toujours les IDs locaux en fallback
    final localCat = prefs.getStringList(_catKey(widget.collection.id)) ?? [];
    catIds.addAll(localCat);
  ...
  List<SavedCard> _sorted(List<SavedCard> cards) {
    final l = [...cards];
    if (_sortBy == 'rarity') l.sort((a, b) => b.rarity.index.compareTo(a.rarity.index));
        ...
        body: Column(children: [
          ...
          Expanded(child: _loading
              ...
              : TabBarView(controller: _tabCtrl, children: [
                  _packTab(p), _cardsTab(), _createTab(p)])),
        ])));
  }
  ...
  Widget _appBar(List<Color> p) => SliverAppBar(
    ...
    flexibleSpace: FlexibleSpaceBar(
      ...
      title: Text(widget.collection.name, style: const TextStyle(
          color: Colors.white, fontWeight: FontWeight.w900, fontSize: 18)),
      background: Stack(children: [
        Positioned.fill(child: Container(decoration: BoxDecoration(
          gradient: LinearGradient(
            begin: Alignment.topLeft, end: Alignment.bottomRight,
            colors: [p[0].withValues(alpha:0.8), p[1].withValues(alpha:0.5),
              const Color(0xFF080814)])))),
        if (widget.collection.imageUrl != null)
          Positioned.fill(child: Opacity(opacity: 0.15,
            child: Image.network(widget.collection.imageUrl!, fit: BoxFit.cover,
  ...
  Widget _chip(String label, IconData icon) => Container(
    ...
    decoration: BoxDecoration(
      color: Colors.black.withValues(alpha:0.4), borderRadius: BorderRadius.circular(8),
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        ...
        child: Column(children: [
          Text('CODE', style: TextStyle(color: Colors.white.withValues(alpha:0.4),
              fontSize: 10, letterSpacing: 3)),
          const SizedBox(height: 8),
          ShaderMask(
            shaderCallback: (b) => LinearGradient(colors: p).createShader(b),
            child: Text(widget.collection.code, style: const TextStyle(
                color: Colors.white, fontSize: 36,
                fontWeight: FontWeight.w900, letterSpacing: 10))),
        ])),
    ]));

  List<Widget> _dropRows() => Rarity.values.reversed.map((r) {
    final rc = _rc(r); final pct = _dropLabels[r]!;
    return Padding(padding: const EdgeInsets.only(bottom: 8),
      child: Row(children: [
        Container(width: 8, height: 8,
          decoration: BoxDecoration(color: rc, shape: BoxShape.circle)),
        const SizedBox(width: 10),
        SizedBox(width: 90, child: Text(_rn(r),
        ...
        Expanded(child: ClipRRect(
          ...
          child: LinearProgressIndicator(
            ...
            valueColor: AlwaysStoppedAnimation(rc.withValues(alpha:0.7)),
            minHeight: 6))),
        const SizedBox(width: 10),
        SizedBox(width: 36, child: Text(pct, textAlign: TextAlign.right,
          style: TextStyle(color: rc, fontSize: 12, fontWeight: FontWeight.bold))),
      ]));
  }).toList();
  ...
  Widget _openBtn(List<Color> p) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: p), borderRadius: BorderRadius.circular(16),
        boxShadow: [BoxShadow(color: p[0].withValues(alpha:0.5),
            blurRadius: 20, offset: const Offset(0, 4))]),
      child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.auto_awesome, color: Colors.white, size: 22),
        SizedBox(width: 10),
        Text('Ouvrir un booster', style: TextStyle(color: Colors.white,
  ...
  Widget _timerWidget() => Container(
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Icon(Icons.hourglass_bottom_rounded, color: Colors.white38, size: 20),
  ...
  // ── Onglet Cartes ──────────────────────────────────────────────────────────
  Widget _cardsTab() => Column(children: [
    Container(
            ...
            return Padding(padding: const EdgeInsets.only(right: 8),
              child: GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: sel ? const Color(0xFF7C3AED).withValues(alpha:0.25)
  ...
  Widget _cardGrid() {
    ...
    if (cards.isEmpty) {
      return Center(child: Column(
        ...
        children: [
          Icon(Icons.style_outlined, size: 56, color: Colors.white.withValues(alpha: 0.1)),
          const SizedBox(height: 12),
    ...
    return GridView.builder(
      ...
      itemBuilder: (_, i) => _CardTile(
        card: cards[i], revealed: obtIds.contains(cards[i].id)));
  }
...
class _CardCreator extends StatefulWidget {
  ...
  const _CardCreator({required this.palette, required this.collectionId,
...
class _CardCreatorState extends State<_CardCreator>
  ...
  // Fond
  int _selectedGrad = -1; // -1 = pas de dégradé
  int _backColor = 0xFF16213E;
  Uint8List? _backImageBytes;

  late AnimationController _legendaryCtrl;

  static const double _cW = 194, _cH = 284; // taille de la zone carte
  ...
  final _backColors = [
    0xFF16213E, 0xFF1A1A2E, 0xFF0F3460, 0xFF533483,
    0xFF2C3E50, 0xFF1B2631, 0xFF4A235A, 0xFF1A5276,
  ];
  ...
  Future<void> _addImage({bool isBack = false}) async {
    final file = await ImagePicker().pickImage(
        source: ImageSource.gallery, maxWidth: 900, imageQuality: 80);
    if (file == null) return;
    final bytes = await file.readAsBytes();
    setState(() {
      if (isBack) { _backImageBytes = bytes; }
      else { _images.add(_ImgLayer(bytes: bytes)); }
    });
  }
  ...
  void _addText() {
    final zone = TextZone(
        text: 'Texte', x: 40, y: 80 + _textZones.length * 30.0,
        fontSize: 13, color: 0xFFFFFFFF);
        ...
        builder: (ctx, setD) => AlertDialog(
          ...
          content: SingleChildScrollView(child: Column(
            ...
            children: [
              ...
              const Text('Taille', style: TextStyle(color: Colors.white70, fontSize: 13)),
              Slider(
                value: fontSize, min: 8, max: 36,
                activeColor: const Color(0xFF7C3AED),
                onChanged: (v) { setD(() => fontSize = v); setState(() => zone.fontSize = v); }),
              const SizedBox(height: 8),
              const Text('Couleur', style: TextStyle(color: Colors.white70, fontSize: 13)),
              const SizedBox(height: 8),
              Wrap(spacing: 8, runSpacing: 8,
                children: [Colors.white, Colors.black, Colors.yellow,
                  Colors.red, Colors.blue, Colors.green,
                  Colors.orange, Colors.purple, Colors.pink,
                  Colors.cyan, Colors.teal, Colors.amber,
                ].map((c) => GestureDetector(
                  onTap: () { setD(() => selColor = c);
                    setState(() => zone.color = c.toARGB32()); },
                  child: Container(width: 28, height: 28,
                    decoration: BoxDecoration(
                      color: c, shape: BoxShape.circle,
                      border: selColor == c
                          ? Border.all(color: Colors.white, width: 2) : null)))).toList()),
            ])),
          actions: [
            TextButton(
              onPressed: () { setState(() => _textZones.removeAt(idx)); Navigator.pop(ctx); },
              child: const Text('Supprimer', style: TextStyle(color: Colors.red))),
            TextButton(
              onPressed: () { setState(() { zone.text = ctrl.text; }); Navigator.pop(ctx); },
      ...
      child: ClipRRect(
        ...
        child: Container(
          ...
          child: Stack(children: [
            // Couches images (empilées)
            ..._images.asMap().entries.map((e) {
              final i = e.key; final layer = e.value;
              return Positioned(
                ...
                child: GestureDetector(
                  ...
                  onTap: () => setState(() {
                    for (var l in _images) { l.selected = false; }
                    layer.selected = !layer.selected;
                  }),
                  child: Stack(children: [
                    Transform.scale(
                      scale: layer.scale, alignment: Alignment.topLeft,
                      child: Image.memory(layer.bytes,
                          width: _cW, fit: BoxFit.fitWidth)),
                    // Bouton supprimer si sélectionné
                    if (layer.selected)
            ...
            ..._textZones.asMap().entries.map((e) {
              final i = e.key; final zone = e.value;
              return Positioned(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(4)),
                    child: Text(zone.text, style: TextStyle(
            ...
            // Nom (draggable)
            Positioned(left: _nameX, top: _nameY,
              child: GestureDetector(
                onPanUpdate: (d) => setState(() {
                ...
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2),
                  child: Text(_nameCtrl.text, style: const TextStyle(
                    color: Colors.white, fontSize: 13, fontWeight: FontWeight.bold,
                    shadows: [Shadow(color: Colors.black, blurRadius: 4)]))))),

            // Rareté (draggable)
            Positioned(left: _rarityX, top: _rarityY,
              child: GestureDetector(
                onPanUpdate: (d) => setState(() {
                ...
                child: Container(
                  ...
                  child: Text(_rn(_rarity), style: const TextStyle(
                      color: Colors.white, fontSize: 8,
                      fontWeight: FontWeight.bold))))),
          ]))));

    // Bordure légendaire animée
    if (_rarity == Rarity.legendary) {
      return AnimatedBuilder(
        animation: _legendaryCtrl,
        builder: (_, __) => SizedBox(
          width: _cW + 6, height: _cH + 6,
          child: Stack(children: [
            Container(
              width: _cW + 6, height: _cH + 6,
              decoration: BoxDecoration(
                ...
                gradient: SweepGradient(
                  ...
                  colors: const [
                    Color(0xFFFFD700), Color(0xFFFFF9C4), Color(0xFFFF8F00),
                    Color(0xFFFFE082), Color(0xFFFFF176), Color(0xFFFFD700)]))),
            Center(child: inner),
          ])));
    }

    return SizedBox(width: _cW + 6, height: _cH + 6,
      child: Stack(children: [
        Container(width: _cW + 6, height: _cH + 6,
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(16),
            border: Border.all(color: rc, width: 3),
            boxShadow: [BoxShadow(color: rc.withValues(alpha:0.45), blurRadius: 14)])),
        Center(child: inner),
      ]));
  }
    ...
    child: ClipRRect(
      ...
      child: Stack(children: [
        if (_backImageBytes != null)
          Positioned.fill(child: Image.memory(_backImageBytes!, fit: BoxFit.cover)),
        if (_backImageBytes == null)
          const Center(child: Opacity(opacity: 0.2,
  ...
  Future<void> _save() async {
    ...
    try {
      // Image principale = première couche (si dispo)
      final primaryBytes = _images.isNotEmpty ? _images.first.bytes : null;
      final extra = _images.length > 1
          ? _images.sublist(1).map((l) => ExtraImage(
              bytes: l.bytes, x: l.x, y: l.y, scale: l.scale)).toList()
          : <ExtraImage>[];

      final card = SavedCard(
        id: '${DateTime.now().millisecondsSinceEpoch}_${math.Random().nextInt(99999)}',
        ...
        nameX: _nameX, nameY: _nameY,
        rarityX: _rarityX, rarityY: _rarityY,
      ...
      try {
        await CollectionService.instance.addCardToCollection(
            widget.collectionId, card.id, card.name, _rn(_rarity));
        supabaseOk = true;
      } catch (e) { debugPrint('Supabase link: $e'); }

      // 3. Fallback local si Supabase échoue
      if (!supabaseOk) {
        final prefs = await SharedPreferences.getInstance();
      ...
      setState(() {
        ...
        _images.clear(); _textZones.clear();
        _backImageBytes = null;
        _nameX = 8; _nameY = 200;
        _rarityX = 8; _rarityY = 222;
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          _toggleBtn('Recto', !_showBack, () => setState(() => _showBack = false)),
          const SizedBox(width: 8),
          _toggleBtn('Verso', _showBack, () => setState(() => _showBack = true)),
        ]),
        ...
        Center(child: TextButton.icon(
          onPressed: () => Navigator.push(context, MaterialPageRoute(
            builder: (_) => CardInspectorScreen(
              frontCard: _buildFront(), backCard: _buildBack()))),
          icon: const Icon(Icons.view_in_ar, color: Colors.white54, size: 16),
          label: const Text('Inspecter en 3D',
        ...
        if (!_showBack) ...[
          ...
          const SizedBox(height: 6),
          Text('💡 Glisse le nom et la rareté directement sur la carte',
          ...
          ...Rarity.values.map((r) {
            final rc = _rc(r); final sel = _rarity == r;
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: sel ? rc.withValues(alpha:0.15) : Colors.white.withValues(alpha:0.04),
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(
                ...
                child: Row(children: [
                  ...
                  SizedBox(width: 80, child: ClipRRect(
                    ...
                    child: LinearProgressIndicator(
                      ...
                      minHeight: 5))),
                  const SizedBox(width: 8),
                  SizedBox(width: 32, child: Text(_dropLabels[r]!,
          ...
          Row(children: [
            Expanded(child: OutlinedButton.icon(
              ...
              label: Text(_images.isEmpty
                  ? 'Ajouter une image' : 'Ajouter une couche',
          ...
          if (_images.isNotEmpty) ...[
            const SizedBox(height: 6),
            Text('Pince pour zoomer • Glisse pour bouger • Tape pour sélectionner/supprimer',
              style: TextStyle(color: Colors.white.withValues(alpha:0.3), fontSize: 11)),
          ],
          const SizedBox(height: 20),
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Ajouter une zone de texte',
          ...
          if (_textZones.isNotEmpty) ...[
            const SizedBox(height: 6),
            Text('Tape sur un texte pour l\'éditer • Glisse pour le déplacer',
              style: TextStyle(color: Colors.white.withValues(alpha:0.3), fontSize: 11)),
          ],
          const SizedBox(height: 20),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: _selectedGrad == -1 ? Colors.white : Colors.white24,
                      width: _selectedGrad == -1 ? 3 : 1)),
                  child: _selectedGrad == -1
              ...
              ...List.generate(_gradients.length, (i) => GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    shape: BoxShape.circle,
                    border: Border.all(
                      color: _selectedGrad == i ? Colors.white : Colors.transparent,
                      width: 3))))),
            ]),
        ] else ...[
          // Verso
          ...
          Wrap(spacing: 12, runSpacing: 12,
            children: _backColors.map((c) => GestureDetector(
              onTap: () => setState(() => _backColor = c),
              child: Container(width: 42, height: 42,
                decoration: BoxDecoration(
                  color: Color(c), shape: BoxShape.circle,
                  border: Border.all(
                    color: _backColor == c ? Colors.white : Colors.white24,
                    width: _backColor == c ? 3 : 1))))).toList()),
          const SizedBox(height: 16),
          SizedBox(width: double.infinity,
            child: OutlinedButton.icon(
              onPressed: () => _addImage(isBack: true),
              icon: const Icon(Icons.photo_library_rounded, color: Colors.white70),
              label: Text(_backImageBytes != null
                  ? '✅ Image verso — changer' : 'Image verso (optionnel)',
                style: const TextStyle(color: Colors.white70)),
              style: OutlinedButton.styleFrom(
                side: BorderSide(color: Colors.white.withValues(alpha:0.2)),
                padding: const EdgeInsets.symmetric(vertical: 12),
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(14),
                boxShadow: [BoxShadow(color: widget.palette[0].withValues(alpha:0.5),
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: active
              ? Colors.transparent : Colors.white.withValues(alpha:0.1))),
          ...
          child: Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: _rc.withValues(alpha:0.85), borderRadius: BorderRadius.circular(3)),
